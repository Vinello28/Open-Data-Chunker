...
\begin{itemize}
    \item \textbf{Modello per Classificazione Binaria:} \texttt{dbmdz/bert-base-italian-xxl-cased}. Sfruttando un modello BERT specificamente pre-addestrato sulla lingua italiana, si ottiene un'elevata precisione nell'interpretazione del contesto semantico nazionale.
    \item \textbf{Modello per Classificazione Multiclasse:} \texttt{FacebookAI/xlm-roberta-large}. Scelto perchè il training set costruito è in lingua inglese; questo modello supporta e gestisce nativamente l'inferenza e l'addestramento multilingua.
    \item \textbf{High-Performance Backend:} Il modello allenato (anche tramite knowledge distillation da grandi LLM) è esportato e servito mediante \textbf{PyTorch (compilato con Dynamo)} con supporto all'accelerazione GPU (CUDA). Ciò abbatte i tempi di inferenza a frazioni di millisecondo per iterazione.
